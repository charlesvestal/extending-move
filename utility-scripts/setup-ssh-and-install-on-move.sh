...
if [[ ! "$setup_autostart" =~ ^[Yy]$ ]]; then
    ...
    echo "You may need to start the server manually: ssh ${REMOTE_USER_ABLETON}@${REMOTE_HOST} 'cd /data/UserData/extending-move && python3 -m gunicorn -c gunicorn_config.py move-webserver:app'"
    exit 0
fi
