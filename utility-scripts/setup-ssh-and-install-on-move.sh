...
if [[ ! "$setup_autostart" =~ ^[Yy]$ ]]; then
    ...
    echo "You may need to start the server manually: ssh ${REMOTE_USER_ABLETON}@${REMOTE_HOST} 'cd /data/UserData/extending-move && python3 -m gunicorn -c gunicorn_config.py move_webserver:app'"
    exit 0
fi
...
case "$1" in
  start)
    ...
    ;;
  stop)
    pkill -u ableton -f 'gunicorn.*move_webserver:app'
    ;;
  restart)
    $0 stop
    sleep 1
    ...
    ;;
  status)
    if pgrep -u ableton -f 'gunicorn.*move_webserver:app' >/dev/null; then
      echo "Running"
    else
      echo "Not running"
      exit 1
