    ...
    def generate_pad_grid(self, used_ids, color_map):
        ...
        for row in range(4):
            for col in range(8):
                ...
                input_html = f'<input type="radio" id="pad_{num}" name="pad_index" value="{num}" {disabled}>'
                label_html = f'<label for="pad_{num}" class="pad-cell {status}"{style}>{label_text}</label>'
